...
public class LoginSystemActivity extends Activity {
    ...
    @Click(R.id.account_sign_in_button)
    void loginButtonClick(){
        ...
        client.get("http://192.168.1.161:8080/qqt_up/shopjson/userLogin.do", params, new BaseJsonHttpResponseHandler<Result<User, User>>() {


            @Override
            public void onSuccess(int statusCode, Header[] headers, String rawJsonResponse, Result<User, User> response) {
                ...
                if (null != response) {
                    if (response.isSuccess()) {
                       ...
                       // user.setUserCode(u.get("userCode").toString());
                        myapplication.setUser(response.getObj());
                        //数据是使用Intent返回
            ...
            @Override
            public void onFailure(int statusCode, Header[] headers, Throwable throwable, String rawJsonData, Result<User, User> errorResponse) {

            }

            @Override
            protected Result<User, User> parseResponse(String rawJsonData, boolean isFailure) throws Throwable {
               // return new ObjectMapper().readValues(new JsonFactory().createParser(rawJsonData), Result.class).next();
                return gson.fromJson(rawJsonData, new TypeToken<Result<User,User>>(){}.getType());
            }
        });
    }
