...
package com.trade.bluehole.trad.util.photo;
...
public class GPUImageFilterTools {
    ...
    public static void showDialog(final Context context,final OnGpuImageFilterChosenListener listener) {
       ...
       /* filters.addFilter("Blend (Source Over)", FilterType.BLEND_SOURCE_OVER);
        ...
        filters.addFilter("查找 (Amatorka)", FilterType.LOOKUP_AMATORKA);
        filters.addFilter("高斯平滑", FilterType.GAUSSIAN_BLUR);
        filters.addFilter("交叉阴影线", FilterType.CROSSHATCH);

        filters.addFilter("方形模糊", FilterType.BOX_BLUR);
        filters.addFilter("CGA颜色空间", FilterType.CGA_COLORSPACE);
        filters.addFilter("膨胀", FilterType.DILATION);
        filters.addFilter("Kuwahara", FilterType.KUWAHARA);
        filters.addFilter("三原色膨胀", FilterType.RGB_DILATION);
        filters.addFilter("素描", FilterType.SKETCH);
        filters.addFilter("卡通", FilterType.TOON);
        filters.addFilter("光滑卡通", FilterType.SMOOTH_TOON);

        filters.addFilter("突出变形", FilterType.BULGE_DISTORTION);
        filters.addFilter("球面透镜效果", FilterType.GLASS_SPHERE);
        filters.addFilter("模糊", FilterType.HAZE);
        filters.addFilter("滤波器", FilterType.LAPLACIAN);
        filters.addFilter("非极大值抑制", FilterType.NON_MAXIMUM_SUPPRESSION);
        filters.addFilter("球面折射", FilterType.SPHERE_REFRACTION);
        filters.addFilter("漩涡", FilterType.SWIRL);
        filters.addFilter("若像素包容", FilterType.WEAK_PIXEL_INCLUSION);
        filters.addFilter("伪色", FilterType.FALSE_COLOR);

        filters.addFilter("色彩平衡", FilterType.COLOR_BALANCE);

        filters.addFilter("水平分 (中间调整)", FilterType.LEVELS_FILTER_MIN);
